...
import {
    FacilityMarker
} from "~/components/map/facility_marker";
...
export function Map() {
  ...
  const { vendingMachinesPending, vendingMachinesError, data: vendingMachines } = useGetAllVendingMachines();
        ...
        {mapsLoaded && (
          ...
          >
            ...
            {vendingMachines.map((vm) => (
              <FacilityMarker key={vm.id} facility={vm} onSelect={() => setSelectedPlace(vm)}/>
            ))}
